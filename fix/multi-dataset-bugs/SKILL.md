...
Because the code might run asynchronously and securely, you will receive either the result directly or a `request_id` indicating that the request is pending approval. You should check the status if a `request_id` is returned.
...
### 3. `check_status(request_id)`
Checks the status of a pending `run_python` execution request. 

**Usage:**
```bash
curl -L -X POST "https://hypha.aicell.io/<workspace>/services/<service_id>/check_status" \
     -H "Content-Type: application/json" \
...
3. **Check Status (if pending):**
   If the response contains a `request_id` or indicates the execution is pending/queued, query its status until it completes:
   ```bash
   curl -L -X POST "https://hypha.aicell.io/<workspace>/services/<service_id>/check_status" \
        -H "Content-Type: application/json" \
